...
StrutsLayout.Datagrid = function Datagrid(in_property, in_styleId, in_styleClass, in_styleClass2, in_allowSelection, in_allowMultipleSelection) {
	...
	function addDatagridCell(row, index, property, styleClass, type, values) {
	    ...
	    if (type=="checkbox") {	   
		   ...
		   if(property.indexOf('initialValues')!=-1)
...
StrutsLayout.addDatagridLine = function(property) {				
	...
	for (i in datagrid.columns) {
		...
		if(document.getElementById("hid")&&document.getElementById("hid").value!=""&&prop.indexOf('initialValues')!=-1){//alert('initialvalue');
		   values=document.getElementById("hid").value;//alert('init val='+values);
		   cellStyle='DATAGRID_SEL';
		  // column.values=values;
		  document.getElementById("hid").value=Number(document.getElementById("hid").value)+1;
		//  alert(document.getElementById("hid").value);
		 }
	//	alert(i+'=iii '+ column.property+'=column.in_property'+column.type +'=column.type '+column.values+'=column.values');
...
StrutsLayout.setDatagridLineState = function(property, state) {
	...
	{
	alert('Veuillez sélectionner la ligne que vous voulez supprimer \nen positionnant le curseur sur un élément de la ligne');
	return;
	}
	...
	for (i in selectedLines) {
		...
		for(var j=0;j<document.forms[0].length;j++){
		 if(document.forms[0].elements[j].name=='datagrid.manifNumConteneurs_OB['+i+']'){
            if(confirm('Etes - vous sûr de vouloir supprimer cette ligne?')){
		      document.forms[0].elements[j].value="";
			  document.forms[0].elements[j+1].value="";
			  document.forms[0].elements[j+2].value="";
			  document.forms[0].elements[j+3].value="";
			}
		 }
		} 
